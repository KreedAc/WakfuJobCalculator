...
export function Calculator({ language, translations: t }: CalculatorProps) {
  ...
  return (
    <div className="max-w-4xl w-full flex flex-col items-center animate-in fade-in duration-500">
      <h1 className="text-4xl md:text-6xl font-extrabold drop-shadow-[0_8px_12px_rgba(0,0,0,0.3)] mb-4 text-center text-transparent bg-clip-text bg-gradient-to-r from-orange-200 via-amber-100 to-red-200">
        {t.title}
      </h1>
      ...
      <form onSubmit={handleCalculate} className="glass rounded-3xl max-w-2xl w-full p-6 md:p-8 space-y-6 transition-all duration-300 hover:shadow-2xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-2">
            <label className="block text-sm font-medium text-orange-200 ml-1">{t.selectProfession}</label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none text-orange-300">
                <Hammer className="w-5 h-5" />
              </div>
              <select
                value={selectedProfession}
                onChange={(e) => setSelectedProfession(e.target.value as ProfessionId)}
                className="glass-soft w-full p-4 pl-12 rounded-xl text-white focus:outline-none focus:ring-2 focus:ring-orange-400/50 appearance-none cursor-pointer transition-all duration-200"
                required
              >
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium text-orange-200 ml-1">{t.selectRange}</label>
            <div className="relative">
              <div className="absolute inset-y-0 left-0 flex items-center pl-4 pointer-events-none text-orange-300">
          ...
          <div className="md:col-span-2 space-y-2">
            <label className="block text-sm font-medium text-orange-200 ml-1">{t.recipe}</label>
            <div className="glass-soft p-4 rounded-xl text-white font-medium flex items-center gap-3">
              <Scroll className="w-5 h-5 text-orange-300" />
              {recipeDisplay}
            </div>
          </div>

          <div className="md:col-span-2 space-y-2">
            <label className="block text-sm font-medium text-orange-200 ml-1">{t.expPerItem}</label>
            <div className="relative">
              <input
                type="number"
                value={expPerItem}
                ...
                className="glass-soft w-full p-4 rounded-xl text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-orange-400/50 transition-all duration-200"
                required
              />
              <div className="absolute inset-y-0 right-0 flex items-center px-4 pointer-events-none text-orange-300/70 text-sm font-medium">
                XP
              </div>
            </div>
          </div>
        ...
        <button
          type="submit"
          className="w-full py-4 rounded-xl font-bold bg-gradient-to-r from-orange-500 to-red-600 hover:from-orange-400 hover:to-red-500 active:from-orange-600 active:to-red-700 text-white shadow-lg transition-all transform hover:-translate-y-0.5 hover:shadow-xl"
        >
          {t.calculate}
        </button>
      </form>
        ...
        <div className="mt-8 glass-strong rounded-3xl shadow-2xl max-w-2xl w-full overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-500">
          <div className="px-8 py-6 border-b border-white/20 bg-gradient-to-r from-white/5 to-white/10">
            <h2 className="text-2xl font-bold text-white flex items-center gap-3 flex-wrap">
              ...
              {t.resultsFor} <span className="text-orange-200">{PROFESSION_NAMES[language][result.selectedProfession]}</span>
          ...
          <div className="p-8">
            <ul className="space-y-4">
              ...
              <li className="flex items-center justify-between">
                <span className="text-orange-200 font-medium">{t.craftsNeeded}</span>
                <span className="font-bold text-3xl text-orange-300 font-mono drop-shadow-sm">
